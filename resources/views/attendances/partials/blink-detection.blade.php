...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        let blinkCount = 0;
        let lastBlinkTime = 0;
        let eyeAspectRatioThreshold = 0.2; // Threshold untuk mata tertutup
        let blinkCountThreshold = 2; // Jumlah kedipan yang diperlukan
        let timeBetweenBlinksThreshold = 2000; // Waktu maksimum antara kedipan (ms)
        ...
        function getUserFaceDescriptor() {
            @if (Auth::user()->profile && isset(Auth::user()->profile['face_descriptor']))
                try {
                    return new Float32Array(JSON.parse('{{ Auth::user()->profile['face_descriptor'] }}'));
                } catch (e) {
        ...
        function updateFaceAnimation(phase) {
            if (blinkDetected) {
                animationText.textContent = "Kedipan terdeteksi!";
                return;
            }
            
            if (blinkCount > 0) {
                animationText.textContent = `Berkedip (${blinkCount}/${blinkCountThreshold})`;
                return;
            }
            
            switch (phase) {
                case 0:
                    animationText.textContent = `Kedipkan mata ${blinkCountThreshold}x untuk verifikasi`;
                    break;
                case 1:
                    animationText.textContent = "Posisikan wajah tepat di tengah";
                    break;
                case 2:
                    animationText.textContent = `Kedipkan mata ${blinkCountThreshold}x berturut-turut`;
                    break;
                case 3:
                    animationText.textContent = "Jangan gerakkan kepala terlalu cepat";
                    break;
                default:
                    animationText.textContent = "Kedipkan mata untuk verifikasi";
            }
        }
            ...
            detectionInterval = setInterval(async () => {
                if (videoElement.readyState === 4 && isModelLoaded && !submissionInProgress) {
                    try {
                        ...
                        // Check for blink
                        const leftEyePoints = [
                            faceLandmarks[36], // left eye left corner
                            faceLandmarks[37], // left eye top left
                            faceLandmarks[38], // left eye top right
                            faceLandmarks[39], // left eye right corner
                            faceLandmarks[40], // left eye bottom right
                            faceLandmarks[41]  // left eye bottom left
                        ];
                        
                        const rightEyePoints = [
                            faceLandmarks[42], // right eye left corner
                            faceLandmarks[43], // right eye top left
                            faceLandmarks[44], // right eye top right
                            faceLandmarks[45], // right eye right corner
                            faceLandmarks[46], // right eye bottom right
                            faceLandmarks[47]  // right eye bottom left
                        ];
                        
                        // Calculate eye aspect ratio (EAR)
                        function calculateEAR(eye) {
                            // Calculate height (average of two measurements)
                            const height1 = distance(eye[1], eye[5]);
                            const height2 = distance(eye[2], eye[4]);
                            const eyeHeight = (height1 + height2) / 2;
                        ...
                        // ctx.fillStyle = "white";
                        // ctx.font = "16px Arial";
                        // ctx.fillText(`EAR: ${avgEAR.toFixed(2)}, Blinks: ${blinkCount}`, 10, 30);
                        
                        if (!eyesClosed && avgEAR < eyeAspectRatioThreshold) {
                            // Eyes just closed
                            eyesClosed = true;
                        } else if (eyesClosed && avgEAR >= eyeAspectRatioThreshold) {
                            // Eyes just opened - blink completed
                            eyesClosed = false;
                            
                            // Check if this is a valid blink (not too long after the previous one)
                            if (currentTime - lastBlinkTime <= timeBetweenBlinksThreshold || blinkCount === 0) {
                                blinkCount++;
                                // Update animation text
                                animationText.textContent = `Berkedip (${blinkCount}/${blinkCountThreshold})`;
                            } else {
                                // Too much time passed, reset blink count
                                blinkCount = 1;
                            }
                            
                            lastBlinkTime = currentTime;
                            
                            // If enough blinks detected
                            if (blinkCount >= blinkCountThreshold && !blinkDetected) {
                                blinkDetected = true;
                                animationText.textContent = "Kedipan terdeteksi!";
                                ...
                                if (userFaceDescriptor) {
                                    ...
                                    const distance = faceapi.euclideanDistance(detectedDescriptor, userFaceDescriptor);
        ...
        async function initialize() {
            ...
            // If running in WebView, show explicit permission button first
            if (isRunningInWebView()) {
                requestLocationPermission();
            } else {
                // Get location for non-WebView
                getLocation();
            }
